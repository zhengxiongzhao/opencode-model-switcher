...
update_models() {
    ...
    FREE_MODELS=()
    while IFS= read -r model; do 
        [ -n "$model" ] && FREE_MODELS+=("opencode/$model")
    done <<< "$(get_free_models)"
    unset IFS

    # Get favorite models with validation
    validate_and_clean_favorites "${FREE_MODELS[@]}"
    ...
    # Helper function: check if element exists in array
    array_contains() {
        local element="$1"
        shift
        local array=("$@")
